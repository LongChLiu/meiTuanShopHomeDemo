...
#import "MeiTuanShopDemo-Swift.h"
...
- (id)initWithFrame:(CGRect)frame  withShopModel:(NewShopModel *)model  withGroupID:(NSString *)groupId currentVC:(UIViewController *)currentVC
{
    ...
    if (self) {
        ...
        self.contentSize = CGSizeMake(self.bounds.size.width, self.frame.size.height + _maxOffset_Y);
        //创建顶部视图，商品,评价,商家
...
- (void)createTopMenuView
{
    ...
    UIView *topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.bounds.size.width, _maxOffset_Y)];
    topView.userInteractionEnabled = YES;
    [self addSubview:topView];
    topView.backgroundColor = [UIColor colorWithWhite:0.5 alpha:0.5];
    ...
    //3:创建 商品,评价,商家
    UIView *menuView = [[UIView alloc]initWithFrame:CGRectMake(0, _maxOffset_Y, self.bounds.size.width, 36)];
    menuView.backgroundColor = [UIColor blueColor];
    [self addSubview:menuView];
    _menuView = menuView;
    //商品
    CGFloat button_W = self.bounds.size.width/3;
    
    
   
    
    UIButton *productBT = [UITool createButtonWithFrame:CGRectMake(0, 0, button_W, menuView.bounds.size.height) Title:@"商品" BgColor:[UIColor whiteColor] TitleColor:[AppMethods colorWithHexStringWithColor:@"333333" :1] Target:self Sel:@selector(btnClick:) Tag:1];
    productBT.titleLabel.font = kFont(14);
    [productBT setBackgroundImage:[AppMethods createImage:[UIColor blueColor]] forState:UIControlStateHighlighted];
    [menuView addSubview:productBT];
    
    //评价
    UIButton *evaluateBT = [UITool createButtonWithFrame:CGRectMake(button_W, 0, button_W, menuView.bounds.size.height) Title:[NSString stringWithFormat:@"评价(%@)",@"6"] BgColor:[UIColor whiteColor] TitleColor:[AppMethods colorWithHexStringWithColor:@"666666" :1] Target:self Sel:@selector(btnClick:) Tag:2];
    evaluateBT.titleLabel.font = kFont(14);
    [evaluateBT setBackgroundImage:[AppMethods createImage:[UIColor blueColor]] forState:UIControlStateHighlighted];
...
//    [_evaluateBT setTitle:[NSString stringWithFormat:@"评价(%@)",[_shopModel.commCount integerValue] >999?@"999+":_shopModel.commCount] forState:UIControlStateNormal] ;
    //商家
    UIButton *merchantBT = [UITool createButtonWithFrame:CGRectMake(button_W*2, 0, button_W, menuView.bounds.size.height) Title:@"商家" BgColor:[UIColor whiteColor] TitleColor:[AppMethods colorWithHexStringWithColor:@"666666" :1] Target:self Sel:@selector(btnClick:) Tag:3];
    merchantBT.titleLabel.font = kFont(14);
    [merchantBT setBackgroundImage:[AppMethods createImage:[UIColor blueColor]] forState:UIControlStateHighlighted];
    [menuView addSubview:merchantBT];
    
    UILabel *line = [UITool lineLab:CGRectMake(0, menuView.bounds.size.height-SINGLE_LINE_WIDTH, self.bounds.size.width, SINGLE_LINE_WIDTH)];;
    line.backgroundColor = [AppMethods colorWithHexStringWithColor:@"E2E2E2" :1];
    [menuView addSubview:line];
    
    //4:可移动的底部滑竿
    _scrollLab = [[UILabel alloc]init];
    _scrollLab.frame = CGRectMake(productBT.center.x-10, menuView.bounds.size.height-2, 20, 2);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (scrollView == self) {
        ...
        if (_shopHomePageView) {
            [_shopHomePageView superScrollViewDidScrollOffsetWithOffset:scrollView.contentOffset.y];
        }
        //发送通知
    ...
    {
        CGFloat scorllOffsetW = self.subScrollView.bounds.size.width*2;
        CGFloat currentOffsetX =  self.subScrollView.contentOffset.x;
        CGFloat rate = currentOffsetX/scorllOffsetW;//速率
        CGFloat offset_X = self.bounds.size.width/3 * rate*2;
        _scrollLab.mj_x = offset_X+(self.bounds.size.width/3/2-10);
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView == self.subScrollView) {
        ...
        [lastBT setTitleColor:[AppMethods colorWithHexStringWithColor:@"666666" :1] forState:UIControlStateNormal];
        [currentBT setTitleColor:[AppMethods colorWithHexStringWithColor:@"333333" :1] forState:UIControlStateNormal];
...
- (void)btnClick:(UIButton *)btn
{
    ...
    UIButton *lastBT = (UIButton *)[_menuView viewWithTag:_lastIndex];
    [lastBT setTitleColor:[AppMethods colorWithHexStringWithColor:@"666666" :1] forState:UIControlStateNormal];
    [btn setTitleColor:[AppMethods colorWithHexStringWithColor:@"333333" :1] forState:UIControlStateNormal];
//    _scrollLab.mj_x = btn.center.x-10;
    ...
    [self.subScrollView setContentOffset:CGPointMake((_lastIndex-1)*self.subScrollView.bounds.size.width, 0) animated:YES];
    self.subTableView = [self currentSubTableView];
}
...
- (void)controlScrollForVertical:(CGFloat)detal AndState:(UIGestureRecognizerState)state {
    ...
    if (self.contentOffset.y >= _maxOffset_Y) {
        ...
        if (offsetY < 0) {
            ...
            offsetY = self.subTableView.contentOffset.y - rubberBandDistance(detal, self.bounds.size.height);
        }
        ...
        if (self.subTableView.contentOffset.y != 0 && detal >= 0) {
            ...
            if (mainOffsetY < 0) {
                ...
                mainOffsetY = self.contentOffset.y - rubberBandDistance(detal, self.bounds.size.height);
...
- (UIScrollView *)currentSubTableView
{
    ...
    switch (_currentIndex) {
        ...
        {
            //店铺主页样式
            if (self.shopViewType == 1) {
                //列表样式
                 tableView = self.shopHomePageView.rightTabView;
            }else if (self.shopViewType == 3){
                //卡片样式
                 tableView = self.shopHomePageView.rightTabView;
            }else{
                //宫格样式
...
- (UIScrollView *)subScrollView {
    if (_subScrollView == nil) {
        _subScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, _menuView.maxY, self.bounds.size.width, self.bounds.size.height-_menuView.maxY+ _maxOffset_Y)];
        _subScrollView.contentSize = CGSizeMake(self.bounds.size.width * 3, _subScrollView.bounds.size.height);
        _subScrollView.pagingEnabled = YES;
        _subScrollView.scrollEnabled = YES;
        _subScrollView.showsHorizontalScrollIndicator = NO;
        _subScrollView.backgroundColor = [AppMethods colorWithHexStringWithColor:@"E2E2E2" :1];
...
{
    if (!_shopHomePageView) {
        _shopHomePageView = [[ShopHomePageView alloc]initWithFrame:CGRectMake(0, 0, self.bounds.size.width, _subScrollView.bounds.size.height)];
        _shopHomePageView.shopSuperView = self;
        _shopHomePageView.currentVC = _currentVC;
        _shopHomePageView.shopModuleType_OC = self.shopViewType;
...
- (ShopEvaluateView *)shopEvaluateView
{
    if (!_shopEvaluateView) {
        _shopEvaluateView = [[ShopEvaluateView alloc] initWithFrame:CGRectMake(self.bounds.size.width, 0, self.bounds.size.width, _subScrollView.bounds.size.height) GroupId:_groupId];
    }
...
{
    if (!_merchantView) {
        _merchantView = [[ShopMerchantView alloc]initWithFrame:CGRectMake(self.bounds.size.width*2, 0, self.bounds.size.width, _subScrollView.bounds.size.height)];
